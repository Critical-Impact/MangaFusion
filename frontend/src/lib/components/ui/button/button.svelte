<script lang="ts" module>
	...
	export const buttonVariants = tv({
		...
		variants: {
			...
			size: {
				...
				mini: "h-auto gap-1 rounded-[var(--r-sm)] border-input px-[0.6rem] py-[0.2rem] text-[0.75rem]",
				icon: "size-8",
	...
	// Internal (in-app) hrefs need svelte-spa-router's `link` action to route via the hash instead
	// of triggering a full browser navigation; external links (with a target, e.g. "_blank", or an
	// absolute URL) must be left alone, since `link` throws on hrefs that aren't rooted at "/".
	function maybeLink(node: HTMLAnchorElement, href: string | undefined) {
...
{#if href}
	<a
		...
		use:maybeLink={restProps.target ? undefined : href}
		{...restProps}
	>
		{@render children?.()}
	</a>
